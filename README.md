...
### Metric dashboards
1. `npm run collect` is used to call the Resource Groups Tagging API and to generate the configuration file.
...
- Node.js 22+ (required by CDK v2 and the TypeScript resource collector)
- CDK v2 (Installation: `npm -g install aws-cdk@latest`)
- AWS credentials configured (for resource collection and deployment)

## Project structure

```
/
├── bin/iem-dashboard.ts              # CDK app entry point
├── src/
│   ├── stacks/                       # metric-dashboard-stack.ts, alarm-dashboard-stack.ts
│   ├── constructs/                   # graph-factory.ts, anomaly-detector-alarm.ts
│   ├── services/                     # 30+ per-service widget set files
│   ├── collector/index.ts            # TypeScript resource collector
│   └── types/                        # config.ts (Zod schema), resource.ts, widget-set.ts, index.ts
├── config/
│   ├── config.json                   # Configuration file
│   └── config.schema.ts              # Zod validation + loadConfig()
├── test/
│   ├── unit/                         # Unit tests (config-validation, graph-factory, services/)
│   ├── snapshot/                     # CDK snapshot tests
│   └── fixtures/                     # Test fixtures
├── functions/                        # Python Lambda functions (alarm dashboard)
├── .github/workflows/ci.yml          # CI workflow
├── .eslintrc.json, .prettierrc.json  # Linting and formatting config
└── package.json, tsconfig.json       # Project configuration
```

## Configuration properties in config/config.json

`BaseName` (String:required) - Base-name of your dashboards. This will be the prefix of the dashboard names.

`ResourceFile` (String:required) - The path for the file where resources are stored. Used by the resource collector
...
4. If deploying for the first time, run `cdk bootstrap` to bootstrap the environment
...
1. Open the configuration file `config/config.json` in your editor of choice.
...
`MetricDashboards.enabled` to false. See _Configuration properties in config/config.json_ above for more information.
6. Save the configuration file.

## Deploying the dashboards

1. If metric dashboards are enabled, run `npm run collect` to create the resource configuration file (`resources.json`
in the `data` directory).
2. **OPTIONAL:** Edit `BaseName` property in `config/config.json` to change the name of your dashboard. In case you plan
to deploy multiple sets of dashboards for different applications in the same account, ensure all subsequent deploys have
different `BaseName`.
...
| `npm run collect` | Run the resource collector |
...
See [DEVELOPING.md](DEVELOPING.md) for detailed instructions on implementing support for new services.

## Tips

Try setting up a repository where you store your code. Set up a CI/CD pipeline to automatically redeploy your dashboard.
...
- Number of Lambda functions per widget is controlled through `CompactMaxResourcesPerWidget` parameter in `config/config.json`
