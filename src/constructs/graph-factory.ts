import { AlarmStatusWidget, Dashboard, Spacer, TextWidget } from 'aws-cdk-lib/aws-cloudwatch';
import { AppsyncWidgetSet } from '../services/appsync';
import { DynamodbWidgetSet } from '../services/dynamodb';
import { Ec2InstancesWidgetSet } from '../services/ec2instances';
import { LambdaWidgetSet } from '../services/lambda';
import { ASGWidgetSet } from '../services/autoscalinggroup';
import { ApiGatewayV1WidgetSet } from '../services/apigatewayv1';
import { ApiGatewayV2WebSocketWidgetSet } from '../services/apigatewayv2websocket';
import { ApiGatewayV2HttpWidgetSet } from '../services/apigatewayv2http';
import { SQSWidgetSet } from '../services/sqs';
import { AuroraWidgetSet } from '../services/aurora';
import { Construct } from 'constructs';
import { ELBv2WidgetSet } from '../services/elbv2';
import { ELBv1WidgetSet } from '../services/elbv1';
import { CapacityReservationsWidgetSet } from '../services/capacityreservations';
import { EcsWidgetSet } from '../services/ecs';
import { TgwWidgetSet } from '../services/tgw';
import { NatgwWidgetSet } from '../services/natgw';
import { SNSWidgetSet } from '../services/sns';
import { WafV2WidgetSet } from '../services/wafv2';
import { CloudfrontWidgetSet } from '../services/cloudfront';
import { LambdaGroupWidgetSet } from '../services/lambdagroup';
import { SQSGroupWidgetSet } from '../services/sqsgroup';
import { S3WidgetSet } from '../services/s3';
import { MediaPackageWidgetSet } from '../services/mediapackage';
import { MediaLiveWidgetSet } from '../services/medialive';
import { EFSWidgetSet } from '../services/efs';
...
export class GraphFactory extends Construct {
  ...
  private generate() {
    const regions = Object.keys(this.serviceArray);
    ...
    for (const region of regions) {
      ...
      const servicekeys = Object.keys(this.serviceArray[region]);
      ...
      for (const servicekey of servicekeys) {
        ...
        switch (servicekey) {
          case 'mediapackage': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              if (resource.Id !== undefined) {
                ...
                const mediapackage = new MediaPackageWidgetSet(
                  this,
                  `MediaPackageWidgetSet-${channelid}-${region}`,
                  resource,
                  this.config,
                );
          ...
          case 'medialive': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              if (resource.id !== undefined) {
                const mlchannelid = resource.ResourceARN.split('/')[resource.ResourceARN.split('/').length - 1];
                const medialive = new MediaLiveWidgetSet(
                  this,
                  `MediaLiveWidgetSet-${mlchannelid}-${region}`,
                  resource,
                  this.config,
                );
          ...
          case 'appsync': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const graphqlendpoint = arn.split('/')[arn.split('/').length - 1];
              const appsync = new AppsyncWidgetSet(this, `AppsyncWidgetSet-${graphqlendpoint}`, resource, this.config);
              if (resourceCount === 0) {
                this.widgetArray.push(appsync.getRegionalMetrics(region, this, this.config));
                resourceCount++;
              }
          ...
          case 'apigatewayv1': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              const apiid = resource.ResourceARN.split('/')[resource.ResourceARN.split('/').length - 1];
              const apigw = new ApiGatewayV1WidgetSet(
                this,
                `APIGWV1WidgetSet-${apiid}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'apigatewayv2': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              if (resource.type === 'WEBSOCKET') {
                ...
                gw = new ApiGatewayV2WebSocketWidgetSet(
                  this,
                  `APIGWV2WebSocketWidgetSet-${resource.apiid}-${region}-${this.config.BaseName}`,
                  resource,
                  this.config,
                );
              } else {
                ...
                gw = new ApiGatewayV2HttpWidgetSet(
                  this,
                  `APIGWV2HTTPWidgetSet-${resource.apiid}-${region}-${this.config.BaseName}`,
                  resource,
                  this.config,
                );
              }
          ...
          case 'dynamodb': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              const tablename = resource.ResourceARN.split('/')[resource.ResourceARN.split('/').length - 1];
              const table = new DynamodbWidgetSet(
                this,
                `DynamoDB-${tablename}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'autoscalinggroup': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              const asgId = resource.split(':')[6];
              const asg = new ASGWidgetSet(
                this,
                `ASGWidgetSet-${asgId}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'sqs': {
            ...
            if (this.config?.Compact) {
              const sqsWidgetSetGroup = new SQSGroupWidgetSet(
                this,
                `SQS-WidgetSet-${region}-${this.config.BaseName}`,
                this.serviceArray[region][servicekey],
                this.config,
              );
              ...
              for (const resource of this.serviceArray[region][servicekey]) {
                const queueName = resource.ResourceARN.split(':')[resource.ResourceARN.split(':').length - 1];
                const sqs = new SQSWidgetSet(
                  this,
                  `SQSWidgetSet-${queueName}-${region}-${this.config.BaseName}`,
                  resource,
                  this.config,
                );
          ...
          case 'aurora': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              const auroraName = resource.ResourceARN.split(':')[resource.ResourceARN.split(':').length - 1];
              const aurora = new AuroraWidgetSet(
                this,
                `AuroraWidgetSet-${auroraName}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'elbv2': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const elbv2 = new ELBv2WidgetSet(
                this,
                `ELBv2WidgetSet-${elbID}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'elbv1': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const elbv1 = new ELBv1WidgetSet(
                this,
                `ELBv1WidgetSet-${elbName}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'odcr': {
            ...
            const odcrs = new CapacityReservationsWidgetSet(
              this,
              `Capacity-res-${region}-${this.config.BaseName}`,
              this.serviceArray[region][servicekey],
              this.config,
            );
          ...
          case 'ecs': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              const clusterName = resource.cluster.clusterName;
              const ecsCluster = new EcsWidgetSet(
                this,
                `ECSCluster-${clusterName}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'tgw': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const tgw = new TgwWidgetSet(
                this,
                `tgw-${tgwId}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'natgw': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const natgw = new NatgwWidgetSet(
                this,
                `natgw-${natgwId}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'sns': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const sns = new SNSWidgetSet(
                this,
                `widgetSetDUB-${topicName}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'wafv2': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const webacl = new WafV2WidgetSet(
                this,
                `widgetSet-${resourceName}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 'cloudfront': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const cfws = new CloudfrontWidgetSet(
                this,
                `cloudfront-${distId}-${region}-${this.config.BaseName}`,
                resource,
                this.config,
              );
          ...
          case 's3': {
            ...
            for (const resource of this.serviceArray[region][servicekey]) {
              ...
              const s3set = new S3WidgetSet(this, `s3-${bucketName}-${this.config.BaseName}`, resource, this.config);
    ...
    if (this.alarmSet.length > 0) {
          ...
          alarm.addAlarmAction(
            new SnsAction(
              sns.Topic.fromTopicArn(
                this,
                `ALARMTOPIC-${alarm.alarmName}-action-${this.config.BaseName}`,
                config.AlarmTopic,
              ),
            ),
          );
      ...
      this.estimatedCost += this.alarmSet.length * 0.1;
  ...
  private sortARNsByService(resources: any[]) {
    for (const resource of resources) {
      ...
      if (
        resource.ResourceARN.includes(':apigateway:') &&
        resource.ResourceARN.includes('/restapis/') &&
        !resource.ResourceARN.includes('stages')
      ) {
        ...
        resource.ResourceARN.includes(':apigateway:') &&
        resource.ResourceARN.includes('/apis/') &&
        !resource.ResourceARN.includes('stages')
      ) {
        ...
        resource.ResourceARN.includes(':autoscaling:') &&
        resource.ResourceARN.includes(':autoScalingGroup:')
      ) {
        ...
        resource.ResourceARN.includes(':rds:') &&
        resource.ResourceARN.includes(':cluster:') &&
        resource.Engine
      ) {
        ...
        resource.ResourceARN.includes(':elasticloadbalancing:') &&
        (resource.ResourceARN.includes('/net/') || resource.ResourceARN.includes('/app/')) &&
        !resource.ResourceARN.includes(':targetgroup/')
      ) {
        ...
        resource.ResourceARN.includes(':elasticloadbalancing:') &&
        !resource.ResourceARN.includes('/net/') &&
        !resource.ResourceARN.includes('/app/') &&
        !resource.ResourceARN.includes(':targetgroup/')
      ) {
  ...
  private processEC2(region: string, servicekey: any) {
    ...
    for (const resource of this.serviceArray[region][servicekey]) {
      ...
      const instance = new Ec2InstancesWidgetSet(
        this,
        `EC2InstancesWidgetSet-${instanceId}-${region}`,
        resource,
        this.config,
      );
        ...
        for (const tag of resource.Tags) {
          if (tag.Key === this.config.GroupingTagKey) {
            ...
            if (this.groupedDashboards.has(tag.Value)) {
              ...
              const dash = new Dashboard(this, this.config.BaseName + '-EC2-Dashboard' + '-' + tag.Value, {
  ...
  private processLambda(region: string, servicekey: any) {
    for (const resource of this.serviceArray[region][servicekey]) {
      const lambda = new LambdaWidgetSet(
        this,
        `Lambda-WS-${resource.Configuration.FunctionName}-${region}-${this.config.BaseName}`,
        resource,
        this.config,
      );

      let lambdaGrouped = false;
      let targetDashboard: Dashboard | undefined;
        ...
        for (const tag of resource.Tags) {
          if (tag.Key === this.config.GroupingTagKey) {
            ...
            if (this.groupedLambdaDashboards.has(tag.Value)) {
              ...
              const dash = new Dashboard(this, this.config.BaseName + '-Lambda-Dashboard' + '-' + tag.Value, {
            ...
            targetDashboard = this.groupedLambdaDashboards.get(tag.Value);
            lambdaGrouped = true;
            break;
          }
        }
      ...
      if (!lambdaGrouped) {
        ...
        targetDashboard = this.LambdaDashboard;
      }

      for (const widget of lambda.getWidgetSets()) {
        if (targetDashboard) targetDashboard.addWidgets(widget);
      }
  ...
  private processCompactLambda(region: string, servicekey: any) {
    ...
    const lambdasPerWidget = Math.min(100, this.config.CompactMaxResourcesPerWidget);

    for (const [key, lambdas] of resourceGroups) {
      console.log(`processing key ${key}`);
      const dashboard: any = new Dashboard(this, `${this.config.BaseName}-Lambda-${key}-${region}`, {
      ...
      if (lambdas) {
        ...
        while (lambdasRemaining > 0) {
          const lambdaIncrement = lambdas.splice(0, lambdasPerWidget);
          const lambdaSet = new LambdaGroupWidgetSet(
            this,
            `Lambdas-${key}-${region}-${offset}-${this.config.BaseName}`,
            lambdaIncrement,
            this.config,
          );
          for (const widget of lambdaSet.getWidgetSets()) {
  ...
  printDashboardCost() {
    ...
    console.log(`ESTIMATED COST IS ${this.estimatedCost.toPrecision(3)}USD/mo`);
    console.log(
      `*****************************************************\n*****************************************************`,
    );
  }
