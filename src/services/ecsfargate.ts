...
export class EcsFargateWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, service: any, clusterName: string, config: any) {
    ...
    const markDown = `***ECS Fargate Service [${serviceName}](https://${region}.console.aws.amazon.com/ecs/home?region=${region}#/clusters/${clusterName}/services/${serviceName}/details) [${clusterName}](https://${region}.console.aws.amazon.com/ecs/home?region=${region}#/clusters/${clusterName}/services) Tasks: ${runningTasks}***`;
    this.widgetSet.push(
      new TextWidget({
        markdown: markDown,
        width: 24,
    ...
    const CPUEC2UtilAlarm = CPUUtilisationMetric.createAlarm(
      this,
