...
export class ApiGatewayV2WebSocketWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, apigw: any, config: any) {
    super(scope, id);
    this.config = config;
    const region = apigw.ResourceARN.split(':')[3];
    ...
    const integrationErrorAlarm = integrationError.createAlarm(
      this,
      `IntegrationErrorAlarm-${apigw.apiid}-${region}-${this.config.BaseName}`,
      {
