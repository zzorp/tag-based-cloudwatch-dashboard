...
export class CapacityReservationsWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    super(scope, id);
    this.config = config;
    const usedMetricsArray = [];
    const availMetricsArray = [];
    ...
    for (const odcr of resource) {
      ...
      const availableMetric = new Metric({
        label: crId,
        namespace: this.namespace,
      ...
      const availableAlarm = availableMetric.createAlarm(this, `${crId}-${this.config.BaseName}`, {
