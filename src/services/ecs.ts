...
export class EcsWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    const markDown = `### ECS Cluster [${clusterName}](https://${region}.console.aws.amazon.com/ecs/home?region=${region}#/clusters/${clusterName}) Tasks: ${runningTasks} Services: ${activeServices}`;
    this.widgetSet.push(
      new TextWidget({
        markdown: markDown,
        width: 24,
    ...
    for (const service of resource.services) {
      if (service.launchType === 'EC2') {
        const ec2serviceWidgetSet = new EcsEC2WidgetSet(
          this,
          `ECSEC2WidgetSet-${service.serviceName}-${region}-${this.config.BaseName}`,
          service,
          clusterName,
          this.config,
        );
        for (const widget of ec2serviceWidgetSet.getWidgetSets()) {
          this.widgetSet.push(widget);
        }
        this.alarmSet = this.alarmSet.concat(ec2serviceWidgetSet.getAlarmSet());
      } else {
        const fargateServiceWidgetSet = new EcsFargateWidgetSet(
          this,
          `FargateWidgetSet-${service.serviceName}-${region}-${this.config.BaseName}`,
          service,
          clusterName,
          this.config,
        );
        for (const widget of fargateServiceWidgetSet.getWidgetSets()) {
          this.widgetSet.push(widget);
        }
