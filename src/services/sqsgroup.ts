...
export class SQSGroupWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resourceArray: any, config: any) {
    ...
    while (queuesremaining > 0) {
      const queueIncrement = resourceArray.splice(0, queuesperwidget);
      ...
      const approximateNumberOfMessagesDelayed = this.getMetricArray(
        queueIncrement,
      ...
      this.widgetSet.push(
        new Row(messageVisibility, messagesSentRecv, messagesDelayedAndOldestMessage, countDeletedEmptyReceives),
      );
