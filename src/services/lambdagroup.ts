  ...
  constructor(scope: Construct, id: string, resourceArray: any, config: any) {
    ...
    for (const metric of errorsMetricArray) {
      if (metric.dimensions && metric.dimensions.FunctionName) {
        const alarm = metric.createAlarm(
          this,
          `Errors-${metric.dimensions.FunctionName}-${region}-${this.config.BaseName}`,
          {
        ...
        if (config.AlarmTopic) {
          alarm.addAlarmAction(
            new SnsAction(
              sns.Topic.fromTopicArn(this, `ALARMTOPIC-${metric.dimensions.FunctionName}`, config.AlarmTopic),
            ),
          );
        }
    ...
    for (const metric of throttlesMetricArray) {
      if (metric.dimensions && metric.dimensions.FunctionName) {
        const alarm = metric.createAlarm(
          this,
          `Throttles-${metric.dimensions.FunctionName}-${region}-${this.config.BaseName}`,
          {
          ...
          alarm.addAlarmAction(
            new SnsAction(
              sns.Topic.fromTopicArn(
                this,
                `ALARMTOPIC-${metric.dimensions.FunctionName}-${metric.metricName}`,
                config.AlarmTopic,
              ),
            ),
          );
    ...
    const concurrentMetricArray = this.getMetricArray(
      resourceArray,
