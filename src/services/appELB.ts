...
export class ApplicationELBWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    const _type = resource.Extras.Type;
    ...
    const unhealthyHostCountMetricArray = [];
    const targetResponseTimeArray = [];
    for (const targetGroup of targetGroups) {
      const targetGroupArn = targetGroup.TargetGroupArn;

      const targetGroupName = targetGroupArn.split(':')[targetGroupArn.split(':').length - 1];

      const unhealthyMetric = new Metric({
        namespace: this.namespace,
      ...
      const targetResponseTime = new Metric({
        namespace: this.namespace,
      ...
      const unhealthyAlarm = unhealthyMetric.createAlarm(
        this,
        `UHAlarm-${targetGroupName}-${region}-${this.config.BaseName}`,
        {
    ...
    const backend5xxAlarm = backend5xxMetric.createAlarm(
      scope,
      `Backend5xxAlarm-${elbName}-${region}-${this.config.BaseName}`,
      {
