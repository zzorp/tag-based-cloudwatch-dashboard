...
export class TgwWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    const markDown = `### TGW [${tgwId}](https://${region}.console.aws.amazon.com/vpc/home?region=${region}#TransitGatewayDetails:transitGatewayId=${tgwId}) Attachments:${resource.attachments.length} Peers:${this.countPeers(resource)}`;
    this.widgetSet.push(
      new TextWidget({
        markdown: markDown,
        width: 24,
    ...
    for (const attachment of resource.attachments) {
      ...
      const markDown = `**Attachment [${attachmentId}](https://${region}.console.aws.amazon.com/vpc/home?region=${region}#TransitGatewayAttachmentDetails:transitGatewayAttachmentId=${attachmentId}) Type:${attachment.ResourceType}${vpcMarkup}**`;
      this.widgetSet.push(
        new TextWidget({
          markdown: markDown,
          width: 24,
