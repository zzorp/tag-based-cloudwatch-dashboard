...
export class NatgwWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    const markDown = `### NATGW [${natgwId}](https://${region}.console.aws.amazon.com/vpc/home?region=${region}#TransitGatewayDetails:transitGatewayId=${natgwId})`;
    this.widgetSet.push(
      new TextWidget({
        markdown: markDown,
        width: 24,
    ...
    const errorPortAllocationAlarm = errorPortAllocationMetric.createAlarm(
      this,
      `errorPortAlarm-${natgwId}-${region}-${this.config.BaseName}`,
      {
    ...
    const packetsDropCountAlarm = packetsDropCountMetric.createAlarm(
      this,
      `packetDropAlarm-${natgwId}-${region}-${this.config.BaseName}`,
      {
