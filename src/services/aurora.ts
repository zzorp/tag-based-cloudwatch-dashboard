...
export class AuroraWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    super(scope, id);
    this.config = config;
    const aurora = resource.ResourceARN.split(':')[resource.ResourceARN.split(':').length - 1];
    const maz = resource.MultiAZ ? ' (MAZ)' : ' (SAZ)';
    ...
    if (resource.DBClusterMembers && resource.DBClusterMembers.length > 0) {
      let counter = 0;
      for (const member of resource.DBClusterMembers) {
        const dbid = member.DBInstanceIdentifier;
        const arnarray = resource.ResourceARN.split(':');
        ...
        const wgt = new RdsWidgetSet(this, 'RDSWidgetSet' + counter, member);
        for (const row of wgt.getWidgetSets()) {
