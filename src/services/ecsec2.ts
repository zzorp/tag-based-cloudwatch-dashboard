...
export class EcsEC2WidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, service: any, clusterName: string, config: any) {
    ...
    const markDown = `***ECS EC2 Service [${serviceName}](https://${region}.console.aws.amazon.com/ecs/home?region=${region}#/clusters/${clusterName}/services/${serviceName}/details) [${clusterName}](https://${region}.console.aws.amazon.com/ecs/home?region=${region}#/clusters/${clusterName}/services) Tasks: ${runningTasks}***`;
    this.widgetSet.push(
      new TextWidget({
        markdown: markDown,
        width: 24,
    ...
    const CPUEC2UtilAlarm = CPUUEc2tilisationMetric.createAlarm(
      this,
      `CPUUtilisationAlarm-${clusterName}-${serviceName}-${this.config.BaseName}`,
      {
    ...
    for (const instance of instances) {
      const markDown = `*Instance [${instance}](https://${region}.console.aws.amazon.com/ec2/v2/home?region=${region}#InstanceDetails:instanceId=${instance})*`;
      this.widgetSet.push(
        new TextWidget({
          markdown: markDown,
          width: 24,
