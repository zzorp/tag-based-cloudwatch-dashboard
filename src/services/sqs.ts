...
export class SQSWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    const queueName = arn.split(':')[arn.split(':').length - 1];
    ...
    if (resource.Attributes) {
      ...
      if (resource.Attributes.RedrivePolicy) {
        const redrivePolicy = JSON.parse(resource.Attributes.RedrivePolicy);
        const dlQueueARN = redrivePolicy.deadLetterTargetArn;
        const dlQueueName = dlQueueARN.split(':')[dlQueueARN.split(':').length - 1];
        const dlAccountId = dlQueueARN.split(':')[4];
        const dlRegion = dlQueueARN.split(':')[3];
