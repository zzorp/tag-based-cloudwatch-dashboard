...
export class LambdaWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    const markDown = `### Lambda [${name}](https://${region}.console.aws.amazon.com/lambda/home?region=${region}#/functions/${functionName}?tab=monitoring) Mem:${memory} RT:${runtime}`;
    ...
    const throttleAlarm = throttleMetric.createAlarm(
      this,
      `Throttles-${functionName}-${region}-${this.config.BaseName}`,
      {
