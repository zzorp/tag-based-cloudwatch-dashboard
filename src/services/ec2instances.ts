...
export class Ec2InstancesWidgetSet extends Construct implements WidgetSet {
  ...
  constructor(scope: Construct, id: string, resource: any, config: any) {
    ...
    let instanceName = '';
    if (resource.Tags) {
      for (const nameTag of resource.Tags) {
        if (nameTag.Key === 'Name') {
    ...
    const markDown = `### Instance${auxdata} [${instanceId}](https://${region}.console.aws.amazon.com/ec2/v2/home?region=${region}#InstanceDetails:instanceId=${instanceId}) ${instanceName} ${instanceType}${burstmodeLabel}/${instanceAz}/Cores:${coreCount}/ThreadsPC:${threadsPerCore}`;
    this.widgetSet.push(
      new TextWidget({
        markdown: markDown,
        width: 24,
    ...
    if (burstable) {
      ...
      const CPUSurplusCreditBalanceAlarm = CPUSurplusCreditBalance.createAlarm(
        this,
        `${instanceId}-CPUCredit-Alarm-${config.BaseName}`,
        {
    ...
    for (const volume of resource.Volumes) {
      const vol = new EbsWidgetSet(this, 'EBSWidgetSet' + volume.VolumeId, volume);
      for (const w of vol.getWidgetSets()) {
